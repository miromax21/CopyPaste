...
final class StandardInteractionController: NSObject, InteractionControlling {
  ...
  private func gestureEnded(translation: CGFloat, velocity: CGFloat) {
    if velocity > 300 || (translation > interactionDistance / 2.0 && velocity > -300) {
      let distanceToTravel = interactionDistance - translation
      finish(initialSpringVelocity: springVelocity(distanceToTravel: distanceToTravel, gestureVelocity: velocity))
    } else {
      cancel(initialSpringVelocity: springVelocity(distanceToTravel: -translation, gestureVelocity: velocity))
    }
  }
  ...
  func update(progress: CGFloat) {
    ...
    if let modalView = presentedViewController.presentationController as? ModalPresentationController {
      modalView.blure.alpha = 1.0 - progress
    }
  }
  ...
  func cancel(initialSpringVelocity: CGFloat) {
    ...
    cancellationAnimator = UIViewPropertyAnimator(
      duration: 0.5,
